...
async function updateAudioMixer(sceneName) {
    ...
    for (const source of audioSources) {
        ...
        muteButton.addEventListener('click', async () => {
            ...
            updateAudioMixer(previewScene); // Refresh mixer to show new state
        });
...
function setSelectedSource(name) {
    selectedSource = name;
    updateSourceList(previewScene); // Re-render to show selection
}
...
sourceMenu.addEventListener('click', async (event) => {
    ...
    try {
        const existingSources = await window.core.getSceneSources(previewScene);
        ...
        console.log(`Added source '${sourceName}' (${sourceId}) to scene '${previewScene}'`);
        await updateSourceList(previewScene);
    } catch (error) {
...
async function addOverlayToScene(overlay) {
    ...
    try {
        ...
        if (brandingSettings.logo) {
            url.searchParams.append('logo', encodeURIComponent(brandingSettings.logo));
        }
