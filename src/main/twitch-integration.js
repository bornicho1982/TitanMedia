...
const TWITCH_SCOPES = [
    ...
    'moderator:read:followers' // Scope required for channel.follow event
].join(' ');
...
class TwitchIntegration {
            ...
            const onWillRedirect = async (event, url) => {
                if (url.startsWith(TWITCH_REDIRECT_URI)) {
                    ...
                    try {
                        ...
                        await this.startEventSubListener();
    ...
    async logout() {
        if (this.eventSubListener) await this.eventSubListener.stop();
        this.apiClient = null;
        this.authProvider = null;
        this.userInfo = null;
        this.disconnectChat();
    ...
    async startEventSubListener() {
        ...
        await this.eventSubListener.subscribeToChannelFollowEvents(this.userInfo.id, (e) => {
            console.log(`${e.userDisplayName} just followed!`);
            this.mainWindow.webContents.send('twitch-event-follow', {
                username: e.userDisplayName
            });
        });
    }

    // --- Utility & Passthrough methods ---
